...
echo "vcftools --gzvcf /projects/mjolnir1/people/qvw641/CottonTop/VCF/Filter/CTT_onlyCTT_filter.vcf.gz --max-missing 0.8 --minDP 4 --keep Samples_onlyCTT --recode --recode-INFO-all --stdout | bcftools query -f '%CHROM\t%POS\t[%GT\t]\n' | python ${script}vcf_to_eigensoft.py > ${outdir}CTT_only_newvcf.eigensoft" >> $jobName1
chmod 755 $jobName1
...
echo "vcftools --gzvcf /projects/mjolnir1/people/qvw641/CottonTop/VCF/Filter/CTT_onlyCTT_filter.vcf.gz --max-missing 0.8 --minDP 4 --keep Samples_onlyCTT  --recode --recode-INFO-all --stdout  |  awk '/^#/{next}{print \"\t\"\$1\":\"\$2\"\t1\t0\t\"\$2\"\t\"\$4\"\t\"\$5 }' > ${outdir}CTT_only_newvcf.snp" >> $jobName2
chmod 755 $jobName2
...
echo "qp3Pop -p ${script}CTT_only_newVCF.par > ${outdir}One_logfile_Sites_CTT_only_newvcf" >> $jobName3  
chmod 755 $jobName3
